...
**两件新东西：全应用的「使用记录」（每个模块的操作都留痕），以及仿真的「参数扫描」。**

### 新增 — 使用记录：每个模块的操作都有时间线

用户的原话是「运行记录不止是实验室记录，还有每次操作的记录，比如我在什么时候写了科研笔记、
什么时候写了摘要，每个模块的使用都有记录」。这需要一张真正的日志表 —— 不能从各模块的时间戳推导：

- 「生成摘要」这类动作**不留痕迹**（AI 结果写在 Note/Paper 上，没有独立时间戳）；
- 编辑会覆盖 `updatedAt`、删除后连行都没了，历史直接消失；
- 跨模块看「这周在做什么」需要一条**共同时间线**，分模块查询拼不出真实先后顺序。

于是新增 `Activity` 表（module / action / title / refId / detail / createdAt），并在 **24 个接口、34 处**埋点：
论文库（入库/更新/删除）、科研笔记（写/改/删）、论文写作、实验管理、选题评估、研究规划（里程碑/周计划）、
组网仿真（跑实验/参数扫描）、文献检索（检索/关键词）、引文关系，以及 **10 个 AI 生成接口**
（摘要、要点、综述草稿、选题打分、空白分析、相关论文、AI 重评优先级、规划助手…）。

四个刻意的取舍：

- **记录永不抛错**：`recordActivity()` 内部吞掉所有异常。记录是**旁路** —— 新表在极老/未迁移的库上还不存在，
  若在这里抛出去，用户的一次「保存笔记」就会因为他根本不知道的原因失败，比少一条记录严重得多。
- **只记有意义的动作**：不把每个 API 调用都记下来（否则时间线被噪声淹没，用户不会再打开它）。
- **标题是人话**：`写了科研笔记「DRL资源分配 - 核心方法笔记」`。`module/action` 用于聚合与筛选，
  但不能替代一句人话；删除类操作**先查一次标题再删**，否则时间线里只剩一串 id。
- **有上限**：90 天 / 5000 行，机会式清理（约每 50 次写入清一次）。不设界的日志表迟早变成一块没人管的石头。

界面：概览页新增「**近 7 天使用记录**」（可按「自定义组件」关闭）——按模块**堆叠柱**看这周时间花在哪、
点柱子看当天逐条明细（时间+模块+动作+标题）、点右侧模块可筛选。**空白天也画出来**，
否则「这周没干活」这件事在图上会消失。读不到表时显示「重启客户端后会自动补上」，而不是一块红叉。

⚠️ **老库自动建表**：桌面端打包后没有 prisma CLI，已装用户靠 `desktop/migrate-database.js` 在启动时补结构。
这次把 DDL（由 `prisma migrate diff` 生成，**不手写**）加进它的 `newTables`，
所以**升级后重启客户端就会自动建出这张表**；打包模板也已重建（新装用户天然带上）。
迁移路径做了**双场景验证**：真实旧库 → `changed:true` 且补出表（改动前还自动留了备份）；模板 → `changed:false`（不白改库）。

### 新增 — 组网仿真：参数扫描

科研上真正要的是**曲线**（交付率随节点数怎么变、队列容量加到多少就饱和），而此前只能手点 N 次「运行实验」
再把数字抄出来、抄错也不知道。引擎本身是确定性的（种子化 + FNV-1a 派生），天生适合批量跑。

- 选轴（拓扑节点数 / 队列容量 / 故障注入时刻 / 检测时延）× 算法（可多选）× 每点种子数 → 一次跑完出曲线；
- **点之前就把规模算清楚**（`预计 2 点 × 1 算法 × 1 种子 = 2 次仿真`），超过上限（轴 ≤12 点、总运行 ≤120 次）
  按钮直接禁用并说明怎么缩，而不是点下去等几分钟才报错；
- **所有指标的曲线一次算好**：切换「看交付率还是看 P95 时延」是瞬间的，不必重跑矩阵；
- 明细表给出均值 / 标准差 / 极值 / 样本数 / 失败数，可**导出 CSV**（服务端附件下载，带 BOM，Excel 直接打开）；
- **逐格落库**（label 前缀「扫描：」）⇒ 扫描自动进入「实验历史」，不必再造一套历史视图；
- **单格失败不中断**：某格引擎报错就记 `status: failed` 继续跑，最后如实报出失败数（半张图 + 明确标记，
  好过整批失败只给一句「仿真失败」）；
- Spine-Leaf + 节点数轴**直接拒绝**（该拓扑规模由 spine/leaf 决定），而不是静默忽略参数。

### 其他

- `PUT /api/papers/[id]` 的字段白名单、自动更新链路修复（generic provider + 退避重试）见 1.3.12；
  本版在此基础上把仿真参数解析抽成 `lib/sim/params.ts` 的**唯一入口**（两条路由共用，避免默认值漂移）。
- 修了两条**桌面迁移测试**的 fixture：它们的「干净库」是手抄的表清单，加了 `Activity` 后就不「干净」了
  ⇒ 改成**从 `migrate-database.js` 的 newTables/newIndexes 派生**，以后再新增表不会假红。
  （测试正确地红了一次 —— 它在干活。）

### 验证

- **生产构建**：`next build` 成功（22.9s，58/58 静态页）—— 这是本机**首次**跑生产构建；CI 一直跑它，
  而 v1.3.8 那次 CI 变红正是它抓到的类型错误（本机 `tsc` 查不出）。
- `prisma validate` 通过；`tsc`、`eslint` 零输出；全量单测 **46 文件 / 840 用例**。
- 本机 e2e **16/16**（含此前刻意跳过的「组网仿真运行实验」写库那条）；
  另外两条真机 spec：使用记录面板 2/2、参数扫描 2/2（真实接口造数据 + 系统 Edge 浏览器验证）。
- **迁移双场景**（见上）；验证写进库的数据已按标记/时间窗清理并复核回基线
  （Paper 20 / Note 1 / SimRun 32 / Activity 0）。
...
复核 v1.3.10 的 CI 时发现的：`e2e (playwright)` 作业失败，报「找不到侧边栏按钮」超时。
而 `release.yml` **不跑 e2e** ⇒ 这个信号整整四版是失效的（同一提交 Release 绿、CI 红，正是本仓库
`PLAYBOOK` 里记过的那类陷阱，这次真的踩上了）。

### 根因：按可见文案找按钮，而文案被改了

v1.3.7 的界面改版把侧边栏文案缩短了 5 处（总览仪表盘→总览、文献检索工具→文献检索、
组网仿真实验→组网仿真、方法论浏览→方法论、系统设置→设置），而 `e2e/*.spec.ts` 用
`getByRole('button', { name: /总览仪表盘/ })` **按文案**找导航项 ⇒ 第一个用例 120s 超时，
`describe.serial` 又把其余 13 个全跳过。CI 时长从 v1.3.6 的 **2m32s** 跳到 v1.3.7 起的 **~9m30s**，
就是这个作业在烧超时（这也是事后能一眼定位的时间线证据）。

### 修法（三层，缺一层都会再犯）

1. **不再依赖可见文案**：`app-shell.tsx` 的导航按钮加 `data-section={item.id}`（桌面端 + 移动端各一处）、
   主内容区加 `data-active-section`；e2e 改用 `[data-section="papers"]` 点击。
2. **断言对齐现实**：e2e 的表里显式区分「侧边栏文案」与「分区标题」—— 这两者**本来就不一样**
   （侧边栏「组网仿真」vs 标题「组网仿真实验」），以前混用一张表，改任何一边都会崩。
3. **本机就能拦住漂移**：新增 `tests/lib/e2e-nav-sync.test.ts`，把 NAV_ITEMS 的 id/文案、
   各分区 `SectionHeader` 的实际标题、e2e 的映射表**逐字对齐**；以后再改文案会在 `npm test` 就红。
   这条是本轮教训的通用形态：**本地跑不了的那套验证，至少把它的「输入」拿回本地校验。**

### 验证

- **本机第一次真跑起了 e2e**（以前做不到：没下载 Playwright 浏览器 + `next dev` 是每目录单例）。
  做法：用**系统 Edge**（`channel: 'msedge'`）绕开浏览器下载；`baseURL` 指向已在跑的 3111 服务
  绕开「第二个 dev server 起不来」；配置留在 `.recon/out/pw-local.config.cjs`（本机资产，不进仓库）。
  结果：`sections.spec.ts` **13/13 通过**（12 个分区全部渲染 + 侧边栏 12 项齐全）；
  `workflows.spec.ts` 中**不写库**的两条 **2/2 通过**（论文库增删自清理、设置页无 Key 引导）。
  会真写库的那条（组网仿真运行实验）留给 CI，不在本机用户库里留记录。
  （注：本机那次 runner **退出码是 1** —— worker 收尾时 Edge 未在 300s 内退出被强杀；
  用例本身 13/13 全过，属沙箱现象，与测试无关。）
- 全量：**41 文件 / 744 用例通过**；`tsc` 无输出、`eslint` 无输出。
- 库复核：e2e 建的临时论文已被用例自身清理（Paper 仍 20 篇、无 `__e2e_playwright_` 残留）。

## [1.3.10] - 2026-09-21

**补上 1.3.9 缺的那一半：AI 能改分数，你也得能改回来。**

1.3.9 上了「AI 重评阅读优先级」并承诺「可人工覆盖」，但发布后自查发现：论文详情里**相关度 / 新颖度是只读的**
（那里的编辑态只服务于笔记）—— 也就是说 AI 把分数写回之后，用户在界面上**根本改不回来**，
承诺的一半是空的。这一版补上入口。

### 修复 — 论文详情可手动改分数与优先级

- 详情页分数旁新增「改分数与优先级」：相关度 / 新颖度（1-10）与优先级（高/中/低）就地可改，
  保存走同一条 `PUT /api/papers/[id]`，并**同时摘掉该篇的「AI 评」标记**（保存后的提示会明说这件事已完成）。
- 保存前用与 API **相同的收敛口径**（`normalizeScore` / `normalizePriority`）——
  不让界面造出 API 会拒绝的值（「界面能填、接口报错」是最招人烦的一类不一致）。
- 详情页写清**这个分数是谁给的**：「这项评分由 AI 按题录与摘要给出（时间）· 你手动改过之后标记会自动摘掉」，
  或者「当前是你自己填的分数」—— 用户不必猜。
- 改完立即刷新来源映射：徽标当场消失，而不是等下次进页面才更新。

### 测试

- `tests/lib/citation-loop.test.ts` 增补 4 例：改分入口存在、保存走同一条 PUT、收敛口径与 API 一致、
  详情页能拿到来源标记且改完刷新。
- 全量：**40 文件 / 738 用例通过**；`tsc` 无输出、`eslint` 无输出。
