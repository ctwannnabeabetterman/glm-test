...
起因是用户的一句反馈：「现在缺一个功能，AI 说的话可以导入嘛，现在各模块是不是没打通」。
查下来两件事都成立：全应用 10 项 AI 能力里只有 4 项能把结果写回某个模块，
其余 6 项的结果只活在前端组件的 `useState` 里（**只有一个「复制」按钮，关掉页面就没了**）；
而 5 处模块之间没有任何代码通路。

### 新增 — 统一的「AI 输出落地」操作栏

每个 AI 面板的结果区现在都有三个动作，替掉原来那个孤零零的「复制」：

- **复制** —— 老行为，留着；
- **存为笔记** —— 直接落成一条 Note（走现成的 `POST /api/notes`，不需要新接口），
  标题自动带来源（`AI 快速摘要 · <论文标题>`），源自论文的还会**自动关联那篇论文**；
- **送到写作** —— 投给写作工作台并**立刻切到写作模块**。

⚠️ 「立刻切页」是刻意的：`draftInbox` 是**一次性投递且不持久化**的（有意为之，见 `lib/store.ts` 的说明）。
如果只投递不切页，用户先去别的模块转一圈、或者顺手关了应用，那段正文就会被静默丢掉 ——
把「投递」和「落点」压在同一次交互里，就不存在丢失窗口了。

覆盖 6 个面板：AI 摘要、摘要生成、方向探索、实验建议、研究空白、相关论文。

### 新增 — 相关论文一键入库

`/api/ai-related-papers` 在 `type='papers'` 时做的是**真实 Crossref 检索**（带真 DOI），
返回的 `{ title, authors, year, venue, doi, url }` 与本项目论文库需要的字段完全对得上 ——
但此前 UI 只给一个 DOI 外链，用户等于拿着可用文献还要手动再录一遍。

现在可多选（默认全选）+ 一键入库，去重交给现成的 `mergeBibliography`（按 DOI / zoteroKey / 标题），
导入后如实报出「新增 N 篇 / 更新 M 篇 / 跳过 K 篇」。

实现上**直接传结构化 `records`，不让前端拼 RIS 文本**：拼文本再让服务端解析回来，
中间要处理换行与转义，任何一处没转干净就**静默丢字段**（少了 DOI 就去重失败、少了作者就填错作者）。
`/api/papers/import` 因此扩展为「`{ text }` 或 `{ records }` 二选一」，并新增 `normalizeRecords()` 做字段收敛
（非对象/无标题的条目丢弃、年份只收有限正数、超长字段截断）。

### 新增 — 笔记 ↔ 论文双向通路

在此之前「这条笔记写的是哪篇论文」在库里**没有任何字段承载** ——
只能靠标题模糊匹配去猜，而实测标题与论文库没有稳定对应关系，猜错会把作者填成别人
（这也是 `KNOWN_ISSUES.md` 里「笔记导出不做按标题模糊兜底」那条的由来）。

- 表结构：`Note.paperIds`（JSON 字符串数组，元素是 `Paper.id`）——
  **照抄同一张表已有的 `topicIds` 成例**，不发明新机制、不另建关联表；
- 笔记详情：新增「关联文献」选择器（带搜索，已关联的排最前），勾选即保存；
- 论文详情：新增「相关笔记」面板，列出挂到这篇论文的笔记，并可「新建并关联」；
- 附带收益：笔记导出 PDF 的作者栏从此**有据可填**，不必再猜。

### 修复 — `PUT /api/notes/[id]` 的字段白名单

原来是 `{ ...body }` **直通**：`id` / `createdAt` / `updatedAt` 都能被前端覆盖，而且不报错 ——
与 v1.3.12 修掉的 `PUT /api/papers/[id]` 是同一个病。现在收敛为显式白名单 + 类型转换
（统一的 `src/lib/notes/payload.ts`，与 `POST` 共用，避免两边各写一份而漂移）。
顺带把两处「脏值写进库就再也修不回来」的入口堵上：JSON 数组列传空串 / 坏 JSON 一律回落 `[]`
（否则读取端的 `JSON.parse` 会抛错，表现为「笔记列表整个打不开」）；`structured` 不再被二次编码。

### 数据库升级说明

`Note.paperIds` 是新列。已装用户靠 `desktop/migrate-database.js` 在启动时按需补列
（桌面端没有 prisma CLI）；打包模板已用 `sync-template.mjs` 对齐并复检。
迁移路径做了双场景验证：**真实旧库备份** → `changed:true` 且补出列、数据无损、幂等；
**打包模板** → `changed:false`（不会每次启动都白改库）。

### 内部质量改动

- **新增守卫 `tests/lib/prisma-client-sync.test.ts`**：把 `schema.prisma` 的每个标量字段
  与 `node_modules/.prisma/client/schema.prisma` 逐个比对。起因是这一版真踩到了
  「改了 schema 却忘了 `prisma generate`」—— 运行时每个新建笔记都 500
  （`Unknown argument 'paperIds'`），而 **`tsc` 与单测都拦不住**：
  字段是从 `Record<string, unknown>` 里展开进 `data` 的（TS 的多余属性检查不看展开进来的键），
  而单测里 `db` 是 mock，根本不走 Prisma 的字段校验。反向验证过这条守卫确实会红。
- **迁移测试的 fixture 改为派生**：`Note`/`Paper`/`Milestone` 的建表语句从 `tableColumns` 生成，
  表/索引/列三样全部自动跟上。此前是手工维护的列清单，每加一列就误报一次
  （2026-09-22 加 `Activity`、2026-09-23 加 `Note.paperIds` 各一次）。
- **`.recon/sync-template.mjs` 补列时优先照抄 schema 的 `@default(...)`**：
  此前一律回落 `DEFAULT ''`，导致 `@default("[]")` 的 JSON 数组列在模板里变成空串 ——
  列名对得上（迁移不比默认值），但默认值语义与 schema 不一致。
- **新增源码守卫**：AI 面板必须接上落地操作栏、笔记↔论文双向入口必须在位；
  漏接会让本机测试变红，而不是等用户发现「AI 说的话又没地方存」。
